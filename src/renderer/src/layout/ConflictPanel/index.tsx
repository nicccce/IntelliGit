...
import { buildConflictRiskReport, buildSyntheticUnifiedDiff, formatFileRiskTitle } from '../../services/conflictRiskService'
...
function ConflictPanel({ isOpen, onClose }: ConflictPanelProps): JSX.Element | null {
  ...
  const handleAbortMerge = useCallback(async () => {
    setLoading(true)
    try {
      await invokeGit('merge.abort')
      setMerging(false)
      setConflictedFiles([])
      setActiveConflictFile(null)
      setBlocks([])
      setSuggestion(null)
      useUiStore.getState().showSuccess('已取消合并')
  ...
  return (<SidePanelShell title="冲突 AI 建议" isOpen={isOpen} onClose={onClose} maxWidth={720}><div className={styles['ig-conflict-panel']}><div className={styles['ig-conflict-meta']}><Tag color="blue"><BranchesOutlined /> 三栏对比</Tag><span>{selectedFileSummary}</span>{hasConflictFile && <Tag color="red">冲突文件</Tag>}{merging && blocks.length > 0 && <Tag color="gold">已解决 {resolvedCount}/{blocks.length} 个冲突块</Tag>}{merging && conflictedFiles.length > 0 && <Tag color="gold">{`${conflictedFiles.length} 个冲突文件`}</Tag>}{merging && conflictedFiles.length === 0 && <Tag color="green">已解决所有冲突</Tag>}{!merging && <Tag>未处于合并状态</Tag>}</div>
      ...
      {merging && conflictedFiles.length === 0 && <div className={styles['ig-conflict-finish-bar']}><span className={styles['ig-conflict-finish-hint']}>所有冲突已解决，点击右侧按钮完成合并并创建提交</span><Button type="primary" icon={<CheckOutlined />} loading={loading} onClick={() => void handleFinishMerge()}>立即合并</Button><Popconfirm title="确认取消合并" description="将放弃当前合并进度并恢复到合并前状态" okText="确认取消" cancelText="返回" okButtonProps={{ danger: true }} onConfirm={() => void handleAbortMerge()}><Button danger loading={loading}>取消合并</Button></Popconfirm></div>}
      {merging && conflictedFiles.length > 0 && <><div className={styles['ig-conflict-finish-bar']}><span className={styles['ig-conflict-finish-hint']}>仍有 {conflictedFiles.length} 个冲突文件待解决</span><Button icon={<CheckOutlined />} loading={loading} disabled>立即合并</Button><Popconfirm title="确认取消合并" description="将放弃当前合并进度并恢复到合并前状态" okText="确认取消" cancelText="返回" okButtonProps={{ danger: true }} onConfirm={() => void handleAbortMerge()}><Button danger loading={loading}>取消合并</Button></Popconfirm></div><div className={styles['ig-conflict-file-list']}>{conflictedFiles.map((f) => <div key={f} className={classNames(styles['ig-conflict-file-item'], activeConflictFile === f && styles['ig-conflict-file-item-active'])} onClick={() => void handleLoadConflictFile(f)}><WarningOutlined className={styles['ig-conflict-file-icon']} /><span className={styles['ig-conflict-file-name']}>{f}</span></div>)}</div></>}
      ...
      {!isBinaryConflict && <><div className={styles['ig-conflict-grid']}>
        <div className={styles['ig-conflict-column']}>
          <div className={styles['ig-conflict-column-title']}><span>ancestor</span><Button size="small" loading={loading} disabled={!ancestor.trim()} onClick={() => void writeResolvedFile(ancestor, '保留 ancestor')}>应用</Button></div>
          <TextArea value={ancestor} onChange={(e) => setAncestor(e.target.value)} placeholder="粘贴共同祖先内容" rows={14} />
        </div>
        <div className={styles['ig-conflict-column']}>
          <div className={styles['ig-conflict-column-title']}><span>ours</span><Button size="small" loading={loading} disabled={!ours} onClick={() => void writeResolvedFile(ours, '保留 ours')}>应用</Button></div>
          <TextArea value={ours} onChange={(e) => setOurs(e.target.value)} placeholder="粘贴当前分支内容" rows={14} />
        </div>
        <div className={styles['ig-conflict-column']}>
          <div className={styles['ig-conflict-column-title']}><span>theirs</span><Button size="small" loading={loading} disabled={!theirs} onClick={() => void writeResolvedFile(theirs, '保留 theirs')}>应用</Button></div>
          <TextArea value={theirs} onChange={(e) => setTheirs(e.target.value)} placeholder="粘贴目标分支内容" rows={14} />
        </div>
      </div><div className={styles['ig-conflict-suggestion']}><div className={styles['ig-conflict-suggestion-title']}><CheckOutlined /> AI 建议{suggestion?.fallback && <Tag color="gold">规则降级</Tag>}{suggestion && <Tag color="blue">{suggestion.strategy}</Tag>}<Button icon={<RobotOutlined />} size="small" loading={suggesting} onClick={() => void handleSuggest()} disabled={!ours && !theirs}>生成建议</Button></div>{(conflictPromptContext || astContext) && <Alert type="info" showIcon icon={<WarningOutlined />} message="已注入 AST 上下文" description={conflictPromptContext || astContext} className={styles['ig-conflict-ast-context']} />}<div className={styles['ig-conflict-suggestion-body']}>{suggestionText}</div>{suggestion?.warnings?.map((warning) => <div key={warning} className={styles['ig-conflict-warning']}>{warning}</div>)}{suggestion?.resolvedContent && <TextArea value={suggestion.resolvedContent} readOnly rows={8} className={styles['ig-conflict-resolved']} />}{suggestion?.resolvedContent && <div className={styles['ig-conflict-copy-row']}><Button onClick={() => void navigator.clipboard.writeText(suggestion.resolvedContent || '')}>复制建议内容</Button><Popconfirm title="将写入文件" description="确定采纳该建议吗？" okText="一键采纳建议" cancelText="取消" onConfirm={() => void writeResolvedFile(suggestion.resolvedContent!, 'AI 智能合并')}><Button type="primary" icon={<CheckOutlined />} loading={loading} disabled={!suggestion.resolvedContent}>一键采纳建议</Button></Popconfirm></div>}{resolvedCount > 0 && <div className={styles['ig-conflict-applied']}>已写入文件</div>}
      <div className={styles['ig-conflict-followup']}><Text strong>追问历史</Text><List size="small" dataSource={followUps} locale={{ emptyText: '暂无追问记录' }} renderItem={(item) => <List.Item><div><div>{item.question}</div><Text type="secondary">{item.answer}</Text></div></List.Item>} /><Input value={followUpQuestion} onChange={(e) => setFollowUpQuestion(e.target.value)} placeholder="为什么不保留 theirs 的版本？" suffix={<Button type="link" icon={<SendOutlined />} loading={suggesting} onClick={() => void handleAskFollowUp()}>发送</Button>} /></div></div></>}</>)}
      <div className={styles['ig-conflict-refresh-bar']}>
        <Button loading={loading} onClick={() => void handleLoadConflict()}>刷新冲突列表</Button>
      </div>
    </div></SidePanelShell>  )
}
