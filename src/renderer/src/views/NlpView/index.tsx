...
import { useGitStatusStore, useHistoryStore, useLlmConfigStore, useRepositoryStore, useUiStore } from '../../store'
...
function NlpView(): JSX.Element {
  const draft = useUiStore(selectNlpDraft)
  const runToken = useUiStore(selectNlpRunToken)
  const setDraft = useUiStore(selectSetNlpDraft)
  const triggerRun = useUiStore(selectTriggerNlpRun)
  ...
  useEffect(() => {
    if (runToken === 0 || handledTokenRef.current === runToken) return
    handledTokenRef.current = runToken
    void parseInput(draft)
  }, [draft, parseInput, runToken])

  const handleRun = useCallback(() => {
    triggerRun(draft.trim())
  }, [draft, triggerRun])
  ...
  return (
    <div className={styles.wrap}>
      <div className={styles.header}>
        <Space>
          <MessageFilled />
          <Typography.Title level={3} style={{ margin: 0 }}>自然语言助手</Typography.Title>
        </Space>
        <Button icon={<ClearOutlined />} onClick={() => setDraft('')}>清空</Button>
      </div>
      ...
      <div className={styles.prompts}>
        {PROMPTS.map((p) => <Tag key={p} className={styles.prompt} onClick={() => triggerRun(p)}>{p}</Tag>)}
      </div>
