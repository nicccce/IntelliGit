...
src/renderer/src/dev/          # SidecarTestPanel 等开发/测试入口（仅调试用，可内聚自己的局部测试状态）
scripts/                       # 构建脚本和边界检查脚本
...
- 正式 UI 需要的组合数据和 action 暴露放 `viewModels/`。
- 不依赖特定 view/store 的通用 React Hook 放 `hooks/`；依赖 store selector 的组合 Hook 放 `viewModels/`。
- 绝不让 `app/MainApp.tsx` 承担页面细节、Git 操作、Diff 渲染或复杂交互。
- 绝不恢复 `src/renderer/src/MainApp.tsx` 根目录转发文件；正式主界面的真实落点是 `src/renderer/src/app/MainApp.tsx`。
- 绝不在 `components/` 中实现完整业务流程。

## 4. 状态与订阅边界

- 必须按领域拆分 Zustand store，通过 `store/index.ts` 统一导出，在 `store/selectors/` 写命名 selector。
- 必须让 `views/`、`layout/`、`components/` 只通过正式 `viewModels/` 消费 store 数据；viewModel 必须通过命名 selector 订阅 store。
- 绝不在 `views/`、`layout/`、`components/` 中直接 import store；viewModel 层允许且应该 import store。
- `dev/` 调试入口不得 import 正式 `store/`，但可以在自己的面板目录内维护局部测试 store、selector 和 view model。
...
- 开发测试工具的 store、selector、view model 和 raw client 必须内聚在 `dev/` 对应目录中，禁止从正式 `store/`、`viewModels/`、`api/` 导出。
...
- 原始 `window.electronAPI.invokeGit(...)` 只允许出现在 `api/gitClient.ts` 和明确标记的 dev raw client 中。
