...
后续不建议一次性大重构，最好按低风险顺序逐步整理：

```text
1. 拆 App.tsx，修复 hooks lint 错误
2. 拆 MainApp.tsx，把视图和公共组件放入 views/components
3. 拆 useAppStore.ts，先抽 gitClient，再拆 store slice
4. 调整 useAppStore 订阅方式，减少无谓重渲染
5. 拆 CSS，先分离旧测试面板样式和正式界面样式
6. 拆 Go handlers.go，按业务域拆 handler 文件
7. 建立强类型 Git command map，减少 response.data as ...
8. 重新设计 Sidecar 当前仓库模型，避免多仓库串状态
9. 收敛 go-git / git CLI adapter 边界
10. 调整自动刷新策略
11. 将凭据从普通配置文件中迁出
12. 整理 docs 和二进制产物
```
...
结果未通过，主要是 `App.tsx` 中条件 hooks 导致的 6 个 React hooks 规则错误，另有一批 Prettier 警告。

---

## 15. 总结

这次检查说明 IntelliGit 当前的问题不是“项目坏掉了”，而是已经到了应该系统整理结构的阶段。

前端主界面、全局 store、CSS 和 Sidecar handler 都有相似的症状：功能不断被加进去，但边界还没有同步长出来。

后续按照上面的清单逐项拆分，可以让项目从“功能可用”进一步走向“长期可维护”。
